			...
			<Container spacing={'VerticalXXXL'}>
				...
				<section>
					<button	className={`button ${button.primary}`}
							onClick={ () => window.location = 'mailto:haotran04022005@gmail.com' } >
						{content.buttons.primary.title}
					</button>
					<button className={`button ${button.secondary} leaveSite`}
							onClick={ ()=> window.open("https://www.linkedin.com/in/tranlynhathao/", "_blank") } >
						{content.buttons.secondary.title}
					</button>
          <button className={`button ${hero.resumeButton}`}
              onClick={() => window.open("/resume", "_self")}>
            {content.buttons.resume.title}
          </button>
				</section>
			</Container>
